...
import { MATCH_TAB_LABELS, EVENT_FILTER_LABELS, LIVE_COPY, STAT_LABELS, COVERAGE_LABELS } from '@/shared/labels/matchLabels'
...
export function MatchCenterPage({ inlineFixture, onBack }: MatchCenterProps = {}) {
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!expectedHome || !expectedAway) {
        ...
        setError(LIVE_COPY.detailsUnavailable)
        return
      }
  ...
  if (error || !data) return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="rounded-3xl border border-white/[0.04] bg-white/[0.015] p-10 text-center">
        <p className="text-[14px] text-white/40">{error || LIVE_COPY.matchNotFound}</p>
      </div>
    </div>
  )
  ...
  const last10 = (() => {
    ...
    const phrase = dominant ? `${dominant} cresce nos últimos minutos.` : LIVE_COPY.fewRelevantActions
    ...
    <div className="max-w-5xl mx-auto space-y-5 animate-fadeIn">
      ...
      <nav className="sticky top-0 z-20 -mx-4 px-4 py-2 bg-[#0a0d14]/90 backdrop-blur-md border-b border-white/[0.03]">
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar">
          {(isMatchScheduled ? [['sec-resumo', MATCH_TAB_LABELS.summary], ['sec-pressao', MATCH_TAB_LABELS.preMatch]] : [['sec-resumo', MATCH_TAB_LABELS.summary], ['sec-pressao', MATCH_TAB_LABELS.pressure], ['sec-stats', MATCH_TAB_LABELS.statistics], ['sec-timeline', MATCH_TAB_LABELS.timeline], ['sec-narracao', MATCH_TAB_LABELS.narration], ['sec-elenco', MATCH_TAB_LABELS.lineups]]).map(([id, label]) => (
      ...
      <section id="sec-resumo" className="rounded-[24px] border border-white/[0.04] bg-white/[0.02] p-5">
        ...
        {last10 && (
          <div className="mt-3 pt-3 border-t border-white/[0.04] flex items-center gap-3">
            <span className="text-[9px] font-bold uppercase tracking-wider text-white/20">{LIVE_COPY.lastTen} 10'</span>
            <span className="text-[10px] text-white/40">{last10.phrase}</span>
            <span className="ml-auto text-[9px] tabular-nums text-white/20">{last10.total} {last10.total === 1 ? LIVE_COPY.oneAction : LIVE_COPY.manyActions}</span>
          </div>
        )}
      </section>
      ...
      {commentary.length > 0 && (
        <section id="sec-narracao" className="rounded-[24px] border border-white/[0.04] bg-white/[0.015] p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/25">{LIVE_COPY.liveNarration}</h3>
            <div className="flex flex-wrap gap-1">
              {([['important', EVENT_FILTER_LABELS.important], ['all', EVENT_FILTER_LABELS.all], ['goals', EVENT_FILTER_LABELS.goals], ['cards', EVENT_FILTER_LABELS.cards], ['subs', EVENT_FILTER_LABELS.substitutions], ['shots', EVENT_FILTER_LABELS.shots]] as [NarrationFilter, string][]).map(([key, label]) => (
          ...
          <div ref={narRef} className="max-h-[320px] overflow-y-auto pr-1 scroll-smooth">
            ...
            {filteredCommentary.length === 0 && <p className="text-[10px] text-white/15 py-3 text-center">{LIVE_COPY.noEventsInCategory}</p>}
          </div>
        </section>
      )}
  ...
  return (
    <section className="rounded-[24px] border border-white/[0.04] bg-white/[0.015] p-5 animate-slideUp">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        {shots && (shots.home + shots.away) > 0 && renderDonut(STAT_LABELS.shots, shots.home, shots.away)}
...
function translateStat(l: string): string {
  const m: Record<string, string> = {
    'SHOTS': STAT_LABELS.shots,
    'ON GOAL': STAT_LABELS.shotsOnTarget,
    'On Target %': STAT_LABELS.precision,
    'Blocked Shots': STAT_LABELS.blocked,
    'POSSESSION': STAT_LABELS.possession,
    'possessionPct': STAT_LABELS.possession,
    'Accurate Passes': STAT_LABELS.accuratePasses,
    'Passes': STAT_LABELS.passes,
    'Pass Completion %': STAT_LABELS.passCompletion,
    'Effective Tackles': STAT_LABELS.tackles,
    'Tackles': STAT_LABELS.attempts,
    'Interceptions': STAT_LABELS.interceptions,
    'Saves': STAT_LABELS.saves,
    'Clearances': STAT_LABELS.clearances,
    'Fouls': STAT_LABELS.fouls,
    'foulsCommitted': STAT_LABELS.fouls,
    'Yellow Cards': STAT_LABELS.yellowCards,
    'yellowCards': STAT_LABELS.yellowCards,
    'Red Cards': STAT_LABELS.redCards,
    'redCards': STAT_LABELS.redCards,
    'Corner Kicks': STAT_LABELS.corners,
    'wonCorners': STAT_LABELS.corners,
    'Offsides': STAT_LABELS.offsides,
    'offsides': STAT_LABELS.offsides,
    'totalShots': STAT_LABELS.shots,
    'shotsOnTarget': STAT_LABELS.shotsOnTarget,
  }
  return m[l] || l
}
    ...
    <div className="flex items-center gap-3">
      ...
      {isAdvanced && (
        <div className="flex items-center gap-2 text-[9px] text-white/20">
          <span>{stats.length > 0 ? `✓ ${COVERAGE_LABELS.statistics}` : `✕ ${COVERAGE_LABELS.statistics}`}</span>
          <span>{events.length > 0 ? `✓ ${COVERAGE_LABELS.events}` : `✕ ${COVERAGE_LABELS.events}`}</span>
          <span>{commentary.length > 0 ? `✓ ${COVERAGE_LABELS.narration}` : `✕ ${COVERAGE_LABELS.narration}`}</span>
          <span>{homeRoster.length > 0 ? `✓ ${COVERAGE_LABELS.lineups}` : `✕ ${COVERAGE_LABELS.lineups}`}</span>
        </div>
      )}
    </div>
  )

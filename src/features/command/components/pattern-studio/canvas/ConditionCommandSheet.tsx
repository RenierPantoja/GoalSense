          ...
          {TRIGGER_RECIPES.map(r => {
            ...
            return (
              <div key={r.id} className="rounded-xl border border-white/[0.07] bg-white/[0.014] px-3.5 py-3 flex flex-col">
                ...
                <button onClick={() => applyRecipe(r.conditions as PatternCondition[])} type="button" className="mt-2.5 h-8 rounded-lg text-[11px] font-semibold text-cyan-200 bg-cyan-500/12 border border-cyan-400/20 hover:bg-cyan-500/20 transition-colors">Aplicar</button>
              </div>
            )
          })}
        </div>

    ...
    <div className="max-w-[1200px] mx-auto space-y-6">
      ...
      {total === 0 && (
        <div className="gs-empty">
          ...
          <p className="text-[15px] text-white/50 font-medium">Nenhum favorito ainda</p>
          <p className="text-[12px] text-white/35 mt-1.5 max-w-[360px] mx-auto">Favorite times, ligas ou partidas para acompanhar de perto e receber sinais personalizados.</p>
          <div className="flex justify-center gap-3 mt-5">
            <button onClick={() => navigate('/app/matches')} className="gs-btn-primary" type="button">Explorar partidas</button>
            <button onClick={() => navigate('/app/leagues')} className="gs-btn-secondary" type="button">Ver ligas</button>
          </div>
        </div>
      )}
      ...
      {teams.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {teams.map(t => (
              <div key={t.id} className="gs-card flex items-center gap-3">
                <ClubLogo src={t.logo} name={t.name} size={32} />
                <span className="text-[14px] font-medium text-white/70 flex-1">{t.name}</span>
                <button onClick={() => toggleFavoriteTeam({ name: t.name, logo: t.logo })} className="text-white/25 hover:text-rose-400/60 transition-colors p-1" type="button"><X size={14} /></button>
              </div>
            ))}
          </div>
        </section>

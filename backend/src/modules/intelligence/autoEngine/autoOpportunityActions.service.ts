...
export async function searchAutoOpportunities(filters: OpportunitySearchFilters): Promise<{
  ...
  userStates: Record<string, { saved: boolean; dismissed: boolean; lastFeedback: string | null; noteCount: number; hasPromotionPlan: boolean }>
}> {
  const repos = createRepositories()
  ...
  const userStates: Record<string, { saved: boolean; dismissed: boolean; lastFeedback: string | null; noteCount: number; hasPromotionPlan: boolean }> = {}
  for (const o of items) {
    const s = stateById.get(o.id)
    if (s) userStates[o.id] = { saved: s.saved, dismissed: s.dismissed, lastFeedback: s.lastFeedback, noteCount: s.noteCount, hasPromotionPlan: s.hasPromotionPlan }
  }
